...
// ---- MANUAL ---------------------------------------------------
function SpecRow({ icon, k, kEn, v, accent }) {
  return (
    <div className="flex items-center gap-4 py-3.5 border-b border-line last:border-0">
      <span className="text-goldDeep/70">{icon}</span>
      <span className="font-gothic text-muted w-20 shrink-0" style={{ fontSize: 11.5 }}>{T(k, kEn)}</span>
...
function SubHead({ children, en }) {
  return (
    <div className="flex flex-col items-center text-center mt-14 sm:mt-24 mb-6 sm:mb-10">
      ...
      <h3 className={`${LANG === 'en' ? 'font-enserif' : 'font-mincho'} text-ink mt-4 sm:mt-6 text-xl sm:text-[22px]`} style={{ letterSpacing: '.05em', fontWeight: 400 }}>{T(children, en)}</h3>
    </div>);
}
...
function ManualSection() {
  return (
    <section id="manual" className="scroll-mt-20 px-5 sm:px-14 py-12 sm:py-24 bg-cream/50">
      ...
      <SectionHead en="Manual" title="映像づくりの、基本ルール" titleEn="The basics of making your video"
      sub="まずは映像を作るときに押さえておきたい、基本の形をご紹介します。"
      subEn="A few essentials to keep in mind before you start editing." center />
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <Reveal className="bg-white border border-line p-6 sm:p-10">
          <div className="flex items-center gap-3.5 sm:gap-4 mb-5 sm:mb-7">
            ...
            <h3 className={`${LANG === 'en' ? 'font-enserif' : 'font-mincho'} text-ink text-base sm:text-[19px]`} style={{ letterSpacing: '.04em' }}>{T('映像の作り方・ルール', 'Video format & rules')}</h3>
          </div>
          <div className="mb-6 sm:mb-8">
            <SpecRow icon={<IconMonitor size={18} />} k="アスペクト比" kEn="Aspect ratio" v="16 : 9" accent />
            <SpecRow icon={<IconClock size={18} />} k="長さの目安" kEn="Length" v="4〜6分 程度" />
          </div>
          {/* aspect do / don't */}
          <div className="grid grid-cols-2 gap-3 sm:gap-5">
            <div>
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 mt-2.5 sm:mt-3 font-gothic text-goldDeep text-[11px] sm:text-[12px]">
                <IconCheck size={14} /> {T('16:9 でぴったり', 'Fits perfectly')}
              </div>
            </div>
            <div>
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 mt-2.5 sm:mt-3 font-gothic text-muted text-[11px] sm:text-[12px]">
                <IconClose size={14} /> {T('4:3 だと黒帯・見切れ', 'Black bars & cropping with 4:3')}
              </div>
            </div>
          </div>
        </Reveal>
      ...
      <SubHead en="Two points to keep in mind">映像づくりで気をつけたい、2つのポイント</SubHead>
      <div className="grid grid-cols-1 md:grid-cols-2 border border-line md:border-r-0 md:border-b-0">
        ...
        <Reveal className="bg-white border-b md:border-r border-line p-6 sm:p-8 flex flex-col">
          <div className="flex items-baseline gap-3 sm:gap-4 mb-4 sm:mb-6">
            ...
            <h4 className={`${LANG === 'en' ? 'font-enserif' : 'font-mincho'} text-ink text-[15px] sm:text-[16px]`} style={{ letterSpacing: '.03em' }}>
              {T('映像の前後に黒画面を5秒', '5 seconds of black at the start & end')}
            </h4>
          </div>
          <div className="flex h-10 overflow-hidden border border-line mb-2">
            <div className="bg-ink flex items-center justify-center" style={{ width: '20%' }}>
              <span className="font-gothic text-white/70" style={{ fontSize: 9.5 }}>{T('黒 5秒', 'Black 5s')}</span>
            </div>
            <div className="ph-stripes flex-1 flex items-center justify-center">
              <span className="font-mono px-2 py-0.5" style={{ fontSize: 9.5, color: '#9A8C6E', background: 'rgba(251,249,245,.8)' }}>{T('本編', 'Video')}</span>
            </div>
            <div className="bg-ink flex items-center justify-center" style={{ width: '20%' }}>
              <span className="font-gothic text-white/70" style={{ fontSize: 9.5 }}>{T('黒 5秒', 'Black 5s')}</span>
            </div>
          ...
          {LANG === 'en' ? (
            <p className="font-gothic text-ink/70 leading-[1.85] sm:leading-[1.95] mt-3 sm:mt-4 text-[12px]">
              Starting on black lets playback begin smoothly. Without black at the end, the player's
              <span className="text-goldDeep font-medium"> menu screen may show through</span>, or the
              <span className="text-goldDeep font-medium"> last moments of picture and sound may cut off early</span>.
            </p>
          ) : (
            <p className="font-gothic text-ink/70 leading-[1.85] sm:leading-[1.95] mt-3 sm:mt-4 text-[12px]">
        ...
        <Reveal delay={90} className="bg-white border-b md:border-r border-line p-6 sm:p-8 flex flex-col last:border-b-0 md:last:border-b">
          <div className="flex items-baseline gap-3 sm:gap-4 mb-4 sm:mb-6">
            ...
            <h4 className={`${LANG === 'en' ? 'font-enserif' : 'font-mincho'} text-ink text-[15px] sm:text-[16px]`} style={{ letterSpacing: '.03em' }}>
              {T('文字は内側に余裕を', 'Keep text inside the safe area')}
            </h4>
          </div>
          <div className="relative overflow-hidden bg-ink" style={{ aspectRatio: '16/9' }}>
            <div className="absolute border border-dashed border-gold/60 flex items-center justify-center" style={{ inset: '5%' }}>
              <span className="font-gothic text-gold/85 text-center leading-relaxed text-[10px] sm:text-[10.5px]">{T(<>文字は<br />この内側に</>, <>Keep text<br />inside here</>)}</span>
            </div>
          </div>
          {LANG === 'en' ? (
            <p className="font-gothic text-ink/70 leading-[1.85] sm:leading-[1.95] mt-3 sm:mt-4 text-[12px]">
              Your PC screen is projected as-is. Text placed right at the edge may be
              <span className="text-goldDeep font-medium"> partly cut off</span>, so keep it slightly inside the visible area.
            </p>
          ) : (
            <p className="font-gothic text-ink/70 leading-[1.85] sm:leading-[1.95] mt-3 sm:mt-4 text-[12px]">
    ...
    <section id="philosophy" className="scroll-mt-20 px-5 sm:px-14 py-12 sm:py-24">
      <Reveal className="grid grid-cols-1 md:grid-cols-5" style={{ background: '#F4EEE3' }}>
        ...
        <div className="col-span-1 md:col-span-3 px-6 sm:px-14 py-8 sm:py-16">
          <Eyebrow en="Our Philosophy">{T('選曲について', 'About Song Selection')}</Eyebrow>
          {LANG === 'en' ? (
            <h3 className="font-enserif text-ink leading-[1.4] sm:leading-[1.5] mt-5 sm:mt-8 text-xl sm:text-2xl lg:text-[27px]" style={{ letterSpacing: '.02em', fontWeight: 400 }}>
              You don't need to have <span className="text-goldDeep">every song decided.</span>
            </h3>
          ) : (
          ...
          <p className="font-gothic text-ink/70 leading-[1.9] sm:leading-[2.15] mt-5 sm:mt-8 text-[12.5px] sm:text-[13px]">
            {T(
              '近ごろは念入りに調べてからお越しになる方が増え、その熱量や想いをとても嬉しく思っています。事前にお渡ししている「my Favorite songs」に、お好きなアーティストや思い出の映画、普段聴く音楽などを書いていただければ準備はバッチリです。',
              'More couples arrive having researched carefully, and we truly appreciate that enthusiasm. Filling in the "my Favorite Songs" sheet we send in advance — favorite artists, a meaningful film, music you listen to day to day — is all the preparation you need.'
            )}
          </p>
          <p className="font-gothic text-ink/70 leading-[1.9] sm:leading-[2.15] mt-3.5 sm:mt-5 text-[12.5px] sm:text-[13px]">
            {T(
              ...
              "But you don't need to have decided exactly which song plays in which scene."
            )}
          </p>
          <p className="font-gothic text-ink/70 leading-[1.9] sm:leading-[2.15] mt-3.5 sm:mt-5 text-[12.5px] sm:text-[13px]">
            {T(
              'シーンに本当に響く一曲は、おふたりがどんな披露宴にしたいか、ゲストへの想い・会場の広さ・進行のテンポまで見合わせて、はじめてかみ合うものだからです。',
              'The song that truly fits a scene only becomes clear once we understand the reception you want, your feelings toward your guests, the room, and the pacing of the day.'
            )}
          </p>
          <p className="font-gothic text-ink/70 leading-[1.9] sm:leading-[2.15] mt-3.5 sm:mt-5 text-[12.5px] sm:text-[13px]">
            {T(
              '「このアーティストが好き」という種があれば十分。お打ち合わせでは、数多くの現場を重ねてきたミュージックプランナーが、おふたりの「こうしたい」をうかがいながら、一番美しく映える配置をご提案し、一緒にカタチにしていきます。',
              'A simple "we love this artist" is seed enough — our experienced music planners will listen to what you envision and shape the most beautiful arrangement together with you.'
            )}
          </p>
          <p className={`${LANG === 'en' ? 'font-enserif' : 'font-mincho'} text-goldDeep mt-7 sm:mt-10 text-[15px] sm:text-[17px]`} style={{ letterSpacing: '.06em' }}>
            {T('どうぞ、私たちの耳と提案を信じて、リラックスしてお越しください。', 'Please trust our ears and our suggestions, and come relaxed.')}
          </p>
          <a href={`music-guide.html${window.location.search}`}
            ...
            {T('選曲イメージの組み合わせやポイントを見る', 'See music selection examples')}
